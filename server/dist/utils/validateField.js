"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.validateGender = exports.validateFullDob = exports.validateComment = exports.validateDob = exports.validatePhoneNumber = exports.validateEmail = exports.validateName = void 0;
const validateName = (name) => {
    if (!name || typeof name !== 'string') {
        return false;
    }
    if (/<[a-z][\s\S]*>/i.test(name)) {
        return false;
    }
    return true;
};
exports.validateName = validateName;
const validateEmail = (email) => {
    if (!email || typeof email !== 'string' || !email.includes('@')) {
        return false;
    }
    if (/<[a-z][\s\S]*>/i.test(email)) {
        return false;
    }
    return true;
};
exports.validateEmail = validateEmail;
const validatePhoneNumber = (phoneNumber) => {
    // if not a number return false
    if (!Number(phoneNumber))
        return false;
    if (/<[a-z][\s\S]*>/i.test(phoneNumber.toString())) {
        return false;
    }
    return true;
};
exports.validatePhoneNumber = validatePhoneNumber;
// note month input starts from 1
const validateDob = (day, month, year) => {
    if (!day || typeof day !== 'number' || day <= 0 || day > 31)
        return false;
    if (!month || typeof month !== 'number' || month <= 0 || month > 12)
        return false;
    if (!year || typeof year !== 'number' || year < 1900 || year > new Date().getFullYear())
        return false;
    // February
    if (month === 2) {
        if (year % 4 === 0) {
            if (day > 29)
                return false;
        }
        else {
            if (day > 28)
                return false;
        }
    }
    if (month === 4 || month === 6 || month === 9 || month === 11) {
        if (day > 30)
            return false;
    }
    const dob = new Date(year, month - 1, day);
    if (!(dob instanceof Date) || isNaN(dob.getTime()))
        return false;
    return true;
};
exports.validateDob = validateDob;
const validateComment = (textAreaInput) => {
    if (typeof textAreaInput !== 'string')
        return false;
    if (/<[a-z][\s\S]*>/i.test(textAreaInput)) {
        return false;
    }
    return true;
};
exports.validateComment = validateComment;
const validateFullDob = (dob) => {
    if (!(dob instanceof Date) || isNaN(dob.getTime()))
        return false;
    return true;
};
exports.validateFullDob = validateFullDob;
const validateGender = (gender) => {
    if (!gender)
        return false;
    return true;
};
exports.validateGender = validateGender;
